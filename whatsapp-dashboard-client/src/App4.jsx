...
function App() {
  ...
  const sendToAI = async () => {
    ...
    setChat([...chat, { from: 'user', text: prompt }]);
    setUserPrompt('');

    try {
      const res = await axios.post('/api/meta-ai-chat', { prompt });
      const aiReply = res.data.reply;
      setChat(prev => [...prev, { from: 'user', text: prompt }, { from: 'bot', text: aiReply }]);
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto p-6 bg-white bg-opacity-20 backdrop-blur-md rounded-lg shadow-xl">
        ...
        {tab === 'dashboard' && (
          <div className="animate-fadeIn">
            ...
            <h2 className="text-2xl font-bold mb-4 text-white">🧠 Chat with Meta AI</h2>

            <div className="bg-white bg-opacity-30 p-4 rounded h-96 overflow-y-scroll mb-4" id="chat-box">
              {chat.map((msg, i) => (
                <div key={i} className={`my-2 p-2 rounded ${msg.from === 'user' ? 'bg-green-200 text-right' : 'bg-blue-200 text-left'}`}>
                  <strong>{msg.from === 'user' ? 'You' : 'Meta AI'}:</strong> {msg.text}
                </div>
              ))}
            </div>
